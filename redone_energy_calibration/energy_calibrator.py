...
def E_calib(data, temp, save=False, plot=False):
    ...
    for name in data["filenames"]:
        ...
        if save == True:
            ...
            save_name = inst + "_" + sample + "_" + id + "_" + iso_name + "_" + "10cm" + "_" + irr_time + "s_" + st + "_ecalib_dosibinning1_rebin_hist.txt"

            # save_array = np.append(np.array([[1, 1], [1, 1], [1, 1], [1, 1]]), save_array)
            np.savetxt("C:/Users/baier/OneDrive/Uni/Bachelorarbeit/sortme/" + save_name, save_array, delimiter=";")
...
print("")
E_calib(data_dict["pvcebis110"], "lowt", save=False, plot=False)
# print("")
# E_calib(data_dict["ebis110"], "roomt", save=False, plot=False)
# print("")
# E_calib(data_dict["dsf"], "roomt", save=False, plot=True)
